...
world_elevation <- read_stars("data/worldelevation.tif")

# country boundaries
world_vector <- read_sf("data/worldvector.gpkg")

# major cities
world_cities <- read_sf("data/worldcities.gpkg")
...
```{r}
tm_shape(world_vector) + #defines the spatial data to plot
  tm_polygons() + #defines how to plot the polygons
...
tm_shape(world_elevation) + #plotting global elevations
  tm_raster() + #plotting tm raster for raster data 
  tm_shape(world_vector) + #plotting world vector data
  tm_borders() + #adds border data
  tm_shape(world_cities) + #
  tm_dots() +
  tm_text("name", size = .7, col = 'black')
...
tm_shape(world_cities) + # plot world_cities first
  tm_dots() + #adds dots to the plot
  tm_text("name") +
tm_shape(world_elevation) + #elevation
  tm_raster() +
...
tm_shape(world_elevation) + #plotting the shape files 
  tm_raster() + #plotting the raster based on the previous file data
  tm_shape(world_vector) + 
  tm_borders() +
  tm_shape(world_cities, is.main = TRUE) + #plotting world_cities last but set as main shape, but borders based on the data points
  tm_dots() + #using the dot type for plotting 
  tm_text('ძაღლი')
```
...
```{r}
tm_shape(world_elevation, (bbox = c(-15,35,45,65))) + #add bouding to restrict extent of map. xmin,ymin,xmax,ymax
  tm_raster() + #adds the raster layewr
  tm_shape(world_vector) + # the shape based on the world vector
  tm_borders() #the border data
```
...
```{r}
# fill polygon with a color palette representing a variables
tm_shape(world_vector) +
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              ...
              fill.legend = tm_legend(title = "Life expectancy (year)",
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              ...
              fill.legend = tm_legend(title = "life expect (years)"))
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              ...
              fill.legend = tm_legend(title = "life expt (years)"))
...
tm_shape(world_vector) +
  tm_fill() +
  tm_scalebar() + 
  tm_compass(position = c("left", "top")) # if we want to change the needle of north, use north = 180 in the arguement
```
